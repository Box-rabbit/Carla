...
需要特别说明：当前长路线基准主流程不是在线 ASR/NLU 直接闭环控车。`carla_eval/evaluator.py` 中车辆控制由场景控制器 `update_state()` 与 `compute_control()` 执行；语音链路当前主要通过 `RouteAudioRuntime` 按路线进度触发离线语音匹配记录和可选显示窗口。`Voice2SimLingoAdapter` 已预留外部 Voice2SimLingo 命令或 mock 后端接口，但未接入当前 `run_benchmark.py` 主评测流程。
...
当前版本未在主评测流程中实现在线 VAD、降噪、方言识别、N-best 候选裁决或实时 ASR/NLU 控制闭环。`Voice2SimLingoAdapter` 预留 `recognized_text`、`recognized_intents`、`target_speed_max_kmh`、`no_collision`、latency 等字段，但该适配器未接入当前长路线基准主流程。
...
适配器预留字段 `recognized_text`、`recognized_intents`、`target_speed_max_kmh`、`no_collision` 只在 `Voice2SimLingoAdapter` 和 metric schema 中存在，当前主运行日志未完整使用。`urgency_level` 未在代码或配置中找到，后续文档不再使用该字段。
...
|---|---|---|---|---|---|---|
| 语音触发 | route progress、匹配 YAML | active voice event | `carla_eval/simlingo/route_audio_runtime.py` | `RouteAudioRuntime.update()` | `configs/simlingo/route_audio_matches.yaml` | `voice_*` |
| 语音适配接口 | 外部命令或 mock | `recognized_text/intents` 等 | `carla_eval/simlingo/voice2simlingo_adapter.py` | `Voice2SimLingoAdapter.run()` | 待接入 | 当前主日志未用 |
...
当前版本的主要不足是：在线语音闭环尚未接入；语义对齐精度、语音识别准确率和 P95 延时统计脚本未完整实现；传感器数据虽接入但未被三类场景状态机直接消费；动态模态权重、方言鲁棒性、传感器超时、最小风险停车状态机均未形成可追溯实现。后续优化应优先补齐 `Voice2SimLingoAdapter` 到 `evaluator.py` 的接入、`intent_match` 和多模态语义对齐计算、传感器时间戳与超时检测、以及 S13 120ms 应急响应目标下的控制触发延时优化。
