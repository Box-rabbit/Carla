...
3. [pipeline/simlingo_benchmark.md](pipeline/simlingo_benchmark.md)
...
- `pipeline/`: SimLingo/Leaderboard 参考流程与接入边界
- `submission/`: 用户维护的提交材料，不由场景整理流程移动或覆盖

阅读原则：

- 当前真实实现优先以 `configs/scenarios/*.yaml`、`routes/*.xml`、SimLingo 适配路线和 `carla_eval/` 代码为准。
