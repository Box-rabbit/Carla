...
| `S11_basic_control_scene1_5km` | PDF 场景1基础操控：5km 连续驾驶、正常车速50、route 上全部左/右转、左变道、提速80、减速30 | `CARLA Leaderboard`、`LMDrive / LangAuto`、`Bench2Drive` | 长路线 route following、全部转弯完成率、组合动作完成率、无碰撞/无违规统计 | 已落地，需真实 CARLA replay 继续调参 |
...
| 交通路况 | `traffic_conditions.traffic_lights`、`active_traffic_light_state`、`active_stop_line_progress_m` | `CARLA Leaderboard`、`ScenarioRunner` | 场景层保留/配置 traffic light，Agent/VLA 负责红灯停车，评测层统计违规 |
...
| 东风赛题要求 | 本项目当前对应能力 | 备注 |
|---|---|---|
| 基础操控场景 | `S01`、`S02`、`S11`，以及规划中的 `S03/B05` | 已有基础速度控制、变道和 PDF 场景1长路线组合操控，后续补独立短路线转向和停车更完整 |
| 复杂避障场景 | `S04`、`S05`，以及规划中的 `S06/C04` | 已覆盖行人与锥桶绕行，后续补慢车和公交站 |
| 应急响应场景 | `S07`、`S08`，以及规划中的 `S09/E04` | 已覆盖 cut-in 与雨夜危险降速，后续补前车急刹和施工并道 |
| 可复现测试场景 | 已支持配置唯一真源、固定 XML route 或 YAML lane_trace、固定 actor spawn | 符合第一版稳定复现目标 |
| 指标统计脚本 | 已有 runtime metrics、report generator、frame logger；S11 已支持离线语音匹配显示字段写入日志 | 后续继续补充真实 ASR/LMDrive 推理延时指标 |
...
在此基础上，项目将东风赛题要求的三类能力映射为一组可复现、可度量、可扩展的 CARLA 闭环评测场景。当前已完成的 `S01`、`S02`、`S04`、`S05`、`S07`、`S08` 已构成第一版稳定场景样板，`S11` 用于 PDF 场景1基础操控主线，后续新增场景应继续沿用本映射表中的参考逻辑与统一指标体系。
