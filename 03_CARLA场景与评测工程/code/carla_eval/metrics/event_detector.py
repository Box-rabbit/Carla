...
def build_events(cfg, frames):
    ...
    # Optional scenario events may intentionally have no timestamp when their
    # actor never activates. Keep those entries reportable and sort them first.
    return sorted(
        events,
        key=lambda event: (float(event.get("timestamp") or 0.0), event.get("event", "")),
    )
