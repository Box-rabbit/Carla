...
该文件由 `scripts/build_official_leaderboard_routes.sh` 生成，包含 Leaderboard 2.0 路线结构以及 S12/S13 的官方 ScenarioRunner 场景节点。`dongfeng_benchmark.xml` 和 `_lmdrive.xml` 保留作兼容、设计或离线评测用途，不作为官方 evaluator 的默认输入。
